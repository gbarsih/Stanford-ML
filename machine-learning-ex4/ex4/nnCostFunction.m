...
a2 = [ones(m,1) sigmoid(Theta1*X')']; %forward prop
h = sigmoid(Theta2*a2');

yk = zeros(num_labels, m); %reshape into vectors
for i=1:m
  yk(y(i),i)=1;
end
...
Theta1Reg = Theta1(:,2:size(Theta1,2)); %dont regularize biases
...
for i=1:m
	...
	Theta1_grad(:,1) = Theta1_grad(:,1) ./ m; %page 11
